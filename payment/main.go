...
func (p Payment) HandleMessage(message *nsq.Message) error {
	...
	if m.Action == ActionStart {
		m.Action = ActionDone
		m.Service = ServiceOrder

		balance = balance + m.Message.Amount

		messageBody, _ := json.Marshal(m)
		err = p.p.Publish(OrderChannel, messageBody)
		if err != nil {
			log.Printf("error publishing done-message to %s channel", OrderChannel)
			return err
		}
		log.Printf("done message published to channel :%s", OrderChannel)
	}
